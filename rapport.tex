...
Ce projet nous a permis d'explorer différentes techniques d’apprentissage automatique pour prédire la qualité des soudures à partir de paramètres mesurables sans tests destructifs. Malgré les défis posés par les données manquantes et les valeurs aberrantes, nous avons développé un modèle performant en appliquant un prétraitement rigoureux, une analyse en composantes principales et en utilisant l’algorithme XGBoost, qui a offert les meilleures performances.

L’EDA a permis de comprendre les relations entre les variables et d’orienter le choix des modèles. Le développement d’une interface web rend notre modèle accessible, offrant une utilité pratique pour les industriels souhaitant estimer rapidement la qualité des soudures.

En résumé, ce travail illustre le potentiel de l’apprentissage automatique pour améliorer le contrôle qualité dans l’industrie des soudures, ouvrant la voie à des applications similaires dans d’autres domaines nécessitant des prédictions à partir de mesures non destructives.
